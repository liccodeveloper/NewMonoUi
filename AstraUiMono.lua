...
function Library._SectionHeader(tab, name)
    -- Visually distinct: darker bg (C.Bg), 1px bottom border, Fg2 text.
    -- Mirrors nav-section-label from the reference panel.html design.
    ...
    -- Label anchored near the bottom of the wrap
    Inst("TextLabel", {
...
function Library._Dropdown(tab, config)
    ...
    -- FIX: CloseDropdown declared as a local upvalue BEFORE RebuildOptions.
    -- Lua closures capture upvalue slots at function-definition time.
    -- If CloseDropdown were defined after RebuildOptions, the click handler
    -- inside RebuildOptions would hold a nil upvalue on the first call.
    -- Additionally, NOT using `local` makes it a global that every new
    -- dropdown instance overwrites — breaking all previously created ones.
    local CloseDropdown
            ...
            if show then
                ...
                ob.MouseButton1Click:Connect(function()
                    if multiSelect then
                        ...
                        selLbl.Text = GetDisplayText()
                        pcall(callback, selected)
                        -- FIX: call the local upvalue, not a global
                        CloseDropdown()
                    end
                end)
            end
    ...
    -- Now assign the function body. RebuildOptions' closure already captured
    -- the slot above, so single-select close will work correctly.
...
function Library._ColorPicker(tab, config)
    ...
    -- FIX: outside-click closes the picker (was missing entirely before).
    -- Guards against closing during an active drag inside the picker (svDragging/hueDragging).
    tab._lib._connections["cp_outside_" .. tostring(row)] = UserInputService.InputBegan:Connect(function(inp)
        if not expanded then return end
        if inp.UserInputType ~= Enum.UserInputType.MouseButton1
        and inp.UserInputType ~= Enum.UserInputType.Touch then return end
        ...
        if not inPicker and not inRow then ClosePicker() end
    end)
